            ...
            document.querySelectorAll(selectors.elementClass).forEach($se => {
                ...
                if ($se) {
                    ...
                    priceRange.on('slideStop', (value) => {
                        ...
                        $minEl.innerText = currency + (value[0]).toLocaleString();
                        $maxEl.innerText = currency + (value[1]).toLocaleString();
                    });
                }
            });
        }
...
// Load Shop Page
const productContainer = document.querySelector(".product-grid-shop");
...
$(window).on('load', function () {
    ...
    if (category || color || size) {
        ...
        numPerPage = Math.floor(products.flat().length / 5) < 10 ? products.flat().length : Math.floor(products.flat().length / 5);

        const filteredProducts = products.filter(product => {
            const productCategory = product.product.category;
            const productColors = product.colors.map(color => color.name);
            const productSizes = product.sizes.map(size => size.name);
        ...
        loadSortedProductsToContainer(filteredProducts);
        reRenderCartPage();
        reRenderCart();
    } else {
        // Make the AJAX request
        $.ajax({
            url: "/api/shop/all",  // Your Laravel web route
            type: "POST",      // Request type
            data: JSON.stringify({ category, color, size }),  // Request data
...
$(document).on("click", ".js-add-cart", async function (e) {
    addItemToCartDefault(e);
    document.querySelector(".js-open-aside").click();
});
...
$(document).on("click", ".js-cart-item-remove", function (e) {
    ...
    const id = e.target.getAttribute("data-product_id");

    const updatedCart = previousCart.filter(cart => cart.unique_id !== id);
...
$(document).on("click", " .remove_item_cart", function (e) {
    ...
    const id = e.target.closest("a").getAttribute("data-product_id");
...
//         maxFiles: 4,
//         init: function () {
//             this.on("success", function (_, response) {
//                 // Add the id to the array
//                 product_add_images.push(response.id);
//                 // Store the id in the file object so we can access it later
//                 _.id = response.id;
//                 // Convert the array into a string and set the value of the hidden input field
//                 document.querySelector(".image_array").value = product_add_images.join(',');
//             });

//             this.on("removedfile", function (file) {
//                 // Remove the id from the array
//                 product_add_images = product_add_images.filter(id => id !== file.id);
//                 // Convert the array into a string and set the value of the hidden input field
//                 document.querySelector(".image_array").value = product_add_images.join(',');
//             });
//         }
//     });
// }
// Image Add For Product Edit and Product Add

$("#add_image").on("click", (e) => {
    e.preventDefault()
    const randomNumber = Math.ceil(Math.random() * Date.now())
    document.querySelector("#image_drop").innerHTML += `
        <div class="col-6 col-md-4 col-lg-3 p-3">
            <label for="images${randomNumber}" class="image_labels">
                <input type="file" name="images[]" id="images${randomNumber}" class="form-control form_images d-none">
                <img src="/files/photo.png" alt="product_image"
...
$("#image_drop").on("change", ".form_images", function (e) {
    e.preventDefault()
    const file = e.target.files[0]
    const size = file.size
    const acceptedFiles = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'image/heic']
    if (size >= 2000000) {
        swal({
            title: "Error",
            text: "Sorry image size cannot be larger than 2MB",
            icon: "error"
        })
        return false
    }
    if (!acceptedFiles.includes(file.type)) {
        swal({
            title: "Error",
            text: "Sorry image type is not supported, upload (jpg,png,webp) files",
            icon: "error"
        })
        return false
    }
    const url = URL.createObjectURL(file)
    const preview = e.target.closest("div").querySelector(".img_previews")
...
$("#add_product_form").on("submit", function (e) {
    e.preventDefault();
    const imageArray = document.querySelectorAll(".image_labels")
    if (imageArray.length < 2) {
        swal({
            title: "Error",
            text: "Sorry, you need to upload at least 2 images",
            icon: 'error',
            buttons: true
        });
        return;
...
// Image select in the categories admin page
$("[name=category_image]").on('change', function (e) {
    const file = e.target.files[0]
    const acceptedTypes = ['image/gif', 'image/jpeg', 'image/png', 'image/webp'];
    const url = URL.createObjectURL(file)
    const image = document.createElement("img")
    if (file.size > 1000000) {
        swal({
            title: "Error",
            text: "Sorry, you need to upload an image less than 1MB",
            icon: 'error',
            buttons: true
        });
        e.target.value = "";
    ...
    if (!acceptedTypes.includes(file.type)) {
        swal({
            title: "Error",
            text: "Sorry, you need to upload an image file",
            icon: 'error',
            buttons: true
        });
        e.target.value = "";
...
// delete click on the admin page
